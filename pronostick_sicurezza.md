...
| Import Calendario da incolla (15/07/2026) | No | Sì (calendario) | Sì (JSON prodotto da un Project claude.ai esterno, fuori dal controllo di Pronostick) | `importaCalendarioIncollato()` — riusa `saveCalendario()`/`renderCalendario()` esistenti (escaping già coperto), aggiunge validazione esplicita per campo (team1/team2/data/ora/competizione) con righe scartate mostrate a schermo, mai silenziose. Dedup sulla stessa chiave di `cercaPartite()` |
