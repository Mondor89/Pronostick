...
| Import/Export backup JSON | No | Sì (dati locali: storico + calendario dal 14/07/2026) | Sì (file caricato dall'utente) | `processImport()` — verificare che il parsing non esegua codice, solo `JSON.parse`. Retrocompatibile con backup vecchi (solo storico, chiave `data`) |
| Import Calendario da incolla o upload file (15/07/2026) | No | Sì (calendario) | Sì (JSON prodotto da un Project claude.ai esterno o dalla skill Claude Code `cerca-calendario`, entrambi fuori dal controllo di Pronostick) | `importaCalendarioIncollato()` — riusa `saveCalendario()`/`renderCalendario()` esistenti (escaping già coperto), aggiunge validazione esplicita per campo (team1/team2/data/ora/competizione) con righe scartate mostrate a schermo, mai silenziose. Dedup sulla stessa chiave di `cercaPartite()`. L'upload file (`handleCalFileUpload()`, 15/07/2026) legge il file con `FileReader` e lo passa alla stessa identica funzione — nessuna nuova validazione/escaping da mantenere, solo una seconda sorgente per lo stesso testo |
...
| 14/07/2026 | Chiusura Bug Noti: aggiunti `aria-label` con nomi squadra (team1/team2) su bottoni icon-only in `renderCalendario()` e `buildCompactCardDOM()`, passati da `escapeHtml()` — stesso principio dell'invariante #4 esteso al contesto attributo HTML. Estesa la superficie di attacco "Import/Export backup JSON" per includere anche il calendario (D6) | Nessuna nuova invariante introdotta, solo applicazione di quelle esistenti a nuovo codice |
| 15/07/2026 | Aggiunto import Calendario da incolla, primo punto di input che accetta dati strutturati da una fonte esterna al tutto (Project claude.ai separato, non l'API di Pronostick). Riuso delle funzioni di salvataggio/render esistenti copre l'escaping (invariante #4), ma non basta da solo: aggiunta validazione esplicita per campo prima del salvataggio, con errori visibili per riga scartata. Estesa la superficie di attacco con la nuova riga "Import Calendario da incolla" | Fabio ha notato che il riuso delle funzioni esistenti garantiva solo la sicurezza del rendering, non la correttezza del contenuto incollato — gap chiuso con validazione dedicata, patch applicata anche a `CLAUDE.md` (sezione "Import/parsing di dati esterni strutturati") |
| 15/07/2026 | Aggiunto upload file diretto (`handleCalFileUpload()`) come seconda via per la stessa importazione, oltre al copia-incolla. Nessuna nuova invariante: il file letto passa dalla stessa validazione/escaping già esistente, cambia solo la sorgente del testo. Sorgente della ricerca calendario ora doppia (Project claude.ai esterno o skill Claude Code `cerca-calendario` locale) — in entrambi i casi il dato resta "esterno non fidato" agli stessi effetti | Riuso completo della pipeline esistente invece di scrivere un parser separato per il file — stesso principio già applicato al copia-incolla. Primo test end-to-end reale ha mostrato che nomi/date sintatticamente validi possono comunque essere semanticamente sbagliati (vedi `CLAUDE.md`, patch 15/07/2026) — non è un problema di escaping/XSS ma di affidabilità del contenuto, tracciato lì invece che qui |
